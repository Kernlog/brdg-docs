...
- **Via Arbitrum.** Where no native venue serves the route, or a plain bridge to Arbitrum is cheaper, the bridge delivers USDC to BRDG's relayer wallet and the relayer credits the account. Nothing to sign or claim after the source chain; the transfer's `postStep` reports the credit. The relayer's cost is a flat amount already off the quoted figure.
