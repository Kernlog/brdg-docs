...
Node 18+ or any runtime with a global `fetch`. Types are generated from the API's OpenAPI document, so requests and responses match the [API reference](/api-reference/overview) exactly.
