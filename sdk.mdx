...
Source and releases: [github.com/Kernlog/brdg-sdk](https://github.com/Kernlog/brdg-sdk). Node 18+ or any runtime with a global `fetch`. One method per endpoint, typed from the API's OpenAPI document, no runtime dependencies.
...
Every API error is a `BrdgError` carrying the API's `code`, the HTTP `status`, `details` and, on `429`, `retryAfterMs`. Codes are listed on [Errors](/reference/errors).
...
Every request and response type is exported: `import type { QuoteResponse, BuildStep, Transfer } from '@kernlog/brdg-sdk'`.
