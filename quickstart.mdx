...
Base URL: `https://api.bridg.now/v1`. No API key. Amounts are strings in atomic units (`100000000` is 100 USDC).
...
<Steps>
  ...
  <Step title="Sign and submit">
    Sign each step with the user's wallet. On EVM, supply nonce and gas yourself. On Solana, sign `serializedTx` without rebuilding it.
  ...
  <Step title="Track">
    ...
    `status` moves `PENDING_SIGNATURE` → `SUBMITTED` → `PENDING` → `COMPLETED`. Terminal states are `COMPLETED`, `PARTIAL`, `REFUNDED`, `FAILED` and `ABANDONED`. Poll, or subscribe to the [realtime channel](/api-reference/realtime/watch).
  </Step>
</Steps>
