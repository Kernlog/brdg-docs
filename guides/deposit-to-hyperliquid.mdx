...
import { brdg, signEvmStep, account } from './evm';
...
async function main() {
  ...
  await brdg.submitTransfer(build.transferId, { txHash: mainHash!, step: 'main' });
...
- Two kinds of route compete on the same table: venues that credit the account directly, and a route through Arbitrum where BRDG's relayer credits it. Both need one signature, on the source chain.
