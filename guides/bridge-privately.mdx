...
async function main() {
  ...
  const { routes } = await brdg.getRoutes({ fromChain: 'base', toChain: 'arbitrum', privacy: true });
  ...
  const build = await brdg.buildTransfer({ decisionId: quote.decisionId, privacy: true });
