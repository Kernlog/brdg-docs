...
description: Take up to 15 bps on every transfer you send through Bridg. Two fields on the quote.
---
...
- Both fields or neither. `referralBps` is `0` to `15`.
...
- `fees.referral` is `null` when the route earns no fee for anyone.
